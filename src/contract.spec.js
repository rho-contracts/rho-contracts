...
describe('hash', function () {
  ...
  it('wrap wraps fields and fails', function () {
    ...
    expect(() => c.hash(c.fn()).wrap(x).thk(5)).to.throw(
      errors.ContractError,
      /Wrong number/
    )
  })
...
describe('object', function () {
  ...
  it('wrap wraps fields and fails', function () {
    const x = { thk: function () {} }
    expect(() => c.object({ thk: c.fn() }).wrap(x).thk(5)).to.throw(
      errors.ContractError,
      /Wrong number/
    )
  })
  ...
  it('extends fails', function () {
    expect(() =>
      c.object({ x: c.number }).extend({ y: c.string }).check({ x: 5 })
    ).to.throw(errors.ContractError)
  })
})
...
describe('strict', function () {
  ...
  it('passes double strict on a good tuple', function () {
    expect(c.tuple(c.value(10)).strict().strict()).to.passValue([10])
  })
  ...
  it('fails a tuple', function () {
    expect(() => c.tuple(c.value(10)).strict().check([10, 20])).to.throw(
      errors.ContractError
    )
  })
  ...
  it('composes with extend', function () {
    expect(() =>
      c.object({ x: c.number }).strict().extend({ y: c.number }).check({ x: 5 })
    ...
    expect(
      c.object({ x: c.number }).strict().extend({ y: c.number })
    ...
    expect(() =>
      c.object({ x: c.number }).extend({ y: c.number }).strict().check({ x: 5 })
    ).to.throw(errors.ContractError, /required/)
    expect(() =>
      c
        .object({ x: c.number })
    ...
    expect(
      c.object({ x: c.number }).extend({ y: c.number }).strict()
    ).to.passValue({ x: 5, y: 6 })
    expect(() =>
      c
        .object({ x: c.number })
...
describe('fn', function () {
  ...
  const manyIdC = c.fn().extraArgs([c.number]).returns(c.number)
  const thisC = c
    .fn(c.number)
...
describe('fun', function () {
  ...
  const manyIdC = c.fun().extraArgs([c.number]).returns(c.number)
  const thisC = c
    .fun({ y: c.number })
    .thisArg(c.object({ x: c.string }))
    .returns(c.string)

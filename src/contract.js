...
contractObject.closeCycle(
  c
    .object({
      ...
      rename: c
        .method(contractObject, { name: c.string })
        .returns(contractObject)
        .doc(
          'Returns `this` with the name `name`. The new name will be used by `toString`',
      ...
      doc: c
        ...
        .returns(contractObject)
        .doc(
          'Returns `this` with zero or more strings set as the `theDoc` array.',
          '',
      ...
      theDoc: c
        .array(c.string)
        .doc(
          'An array of strings which will be used by `publish` to document the',
          'values being published.'
        ),
    })
    .rename('contractObject')
    .doc(
      'Contracts having following methods. Many values can be promoted to a `contractObject`',
...
var strictExtension = {
  strict: c
    ...
    .returns(contractObject)
    .doc(
      'Given a tuple contract or a object contract, returns a version of that',
...
objectContractObject.closeCycle(
  contractObject
    .extend({
      extend: c
        .method(objectContractObject, { fieldSpec: c.hash(c.contract) })
        .returns(contractObject)
        .doc(
          'Returns a contract like `this` that in addition checks that the fields',
...
functionContract.closeCycle(
  contractObject
    .extend({
      extraArgs: c
        .method(functionContract, { extraArgContract: c.optional(c.contract) })
        .returns(functionContract)
        .doc(
          'Returns a contract like `this` that accepts a variable number of',
      ...
      thisArg: c
        .method(functionContract, { thisContract: c.contract })
        .returns(functionContract)
        .doc(
          'Returns a contract like `this` that accepts only calls where the',
          'implicit `this` argument passes `thisContract`.'
        ),

      returns: c
        .method(functionContract, { resultContract: c.contract })
        .returns(functionContract)
        .doc(
          'Returns a function contract like `this` that accepts only calls that returns a',
          'value that passes `resultContract`.'
        ),

      constructs: c
        .method(functionContract, { fieldContracts: c.hash(c.contract) })
        .returns(functionContract)
        .doc(
          'Returns a function contract like `this` that accepts constructor functions.',
...
var contracts = {
  check: c
    ...
    .doc(
      "Verifies that `data` satisfies `contract`, if it doesn't, throws a `ContractError`,",
  ...
  wrap: c
    ...
    .doc(
      "Like `check`, verifies that `data` satisfies `contract`, if it doesn't, throws a `ContractError`.",
  ...
  optional: c
    .fun({ contract: c.contract })
    .returns(contractObject)
    .doc(
      'Returns an optional version of `contract`. It will accept all falsy values in',
  ...
  any: c.contract.doc('Accepts any value.'),

  pred: c
    .fun({ pred: c.fn(c.any).returns(c.any) })
    .returns(contractObject)
    .doc(
      'Given a function `pred`, accepts all values for which `pred` returns truthy.'
    ),

  nothing: c.contract.doc('Rejects all values.'),

  falsy: c.contract.doc(
    'Accepts only `false`, `null`, `undefined`, the empty string, the',
    'number 0, and the value NaN.'
  ),

  truthy: c.contract.doc(
    'Accepts all values except `false`, `null`, `undefined`, the empty string, the',
    'number 0, and the value NaN.'
  ),
  ...
  string: c.contract.doc('Accepts strings.'),

  number: c.contract.doc('Accepts numbers'),

  integer: c.contract.doc('Accepts integers'),

  bool: c.contract.doc('Accepts only the values `true` and `false`.'),

  regexp: c.contract.doc('Accepts regexps'),

  date: c.contract.doc('Accepts `Date`'),

  error: c.contract.doc('Accepts `Error`'),

  anyFunction: c.contract.doc(
    'Accepts any function. To put contract on the argument and return',
    'value, use `fn`, `fun`, or `method`.'
  ),

  isA: c
    .fun({ parent: c.anyFunction })
    .returns(contractObject)
    .doc(
      'Accepts only values `v` for which `v instanceof parent` returns true.'
    ),

  contract: c.contract.doc(
    'Accept contract object and any values which can be promoted to a',
  ...
  toContract: c
    ...
    .returns(contractObject)
    .doc(
      'Promote the given value to a contract object. Arrays are promoted to an',
  ...
  quacksLike: c
    .fun({ parent: c.any }, { name: c.string })
    .returns(contractObject)
    .doc(
      'Accepts any object which has at least the same fields as `parent`, with the same',
  ...
  silentAnd: c
    ...
    .returns(contractObject)
    .doc(
      'Like `and`, but does not mention the presence of the `and` contract in',
      'the error messages.'
    ),
  ...
  or: c
    ...
    .returns(contractObject)
    .doc(
      'Accepts values which passes at least one of the given contracts.',
  ...
  cyclic: c
    .fun({ needsWrapping: c.optional(c.bool) })
    .returns(contractObject)
    .doc(
      'Returns an empty placeholder contract which can later be populated with .closeCycle(c).',
      '',
  ...
  forwardRef: c
    ...
    .returns(contractObject)
    .doc(
      'Synonym for cyclic(), with `needsWrapping` defaulting to `false`. Uses `setRef` instead of `closeCycle`'
    ),

  array: c
    .fun({ itemContract: c.contract })
    .returns(contractObject)
    .doc(
      'Accepts arrays of any size whose elements are all accepted by `itemContract`.'
    ),

  tuple: c
    .fun()
    .extraArgs([c.contract])
    .returns(tupleContractObject)
    .doc(
      'Accepts array with as many elements as the number of given contract',
  ...
  object: c
    .fun({ fieldContracts: c.optional(c.hash(c.contract)) })
    .returns(objectContractObject)
    .doc(
      'Accepts objects that have at least all the fields specified in',
  ...
  fn: c
    ...
    .returns(functionContract)
    .doc(
      'Accepts function that accepts as many arguments as the number of',
  ...
  method: c
    ...
    .returns(functionContract)
    .doc(
      'Accepts functions that accepts as many arguments as the number of',
  ...
  setErrorMessageInspectionDepth: c
    .fun({ depth: c.integer })
    .doc(
      'Set a depth to pass to `util.inspect()` to limit the size of the data presented in',
  ...
  fromExample: c
    .fun({ value: c.any }, { withQuestionMark: c.optional(c.bool) })
    .returns(contractObject)
    .doc(
      'Returns a contract that accepts values of the same type as `value`.',
      '',
  ...
  documentModule: c
    .fun({ moduleName: c.string })
    .extraArgs(c.array(c.string))
    .doc(
      'Documents the given module in the `documentationTable`.',
      '',
  ...
  documentCategory: c
    .fun({ moduleName: c.string }, { category: c.string })
    .extraArgs(c.array(c.string))
    .doc(
      'Store the additional arguments in the `documentationTable` for `moduleName` and `category`,',
  ...
  documentType: c
    .fun({ moduleName: c.string }, { contract: contractObject })
    .doc(
      'Stores the content of the `theDoc` field of `contract` in the `documentationTable`',
  ...
  wrapAll: c
    .fun({ implementation: c.object() }, { contracts: c.hash(c.contract) })
    .doc(
      'Returns an object like `implementation` where each element is wrapped using the',
  ...
  publish: c
    .fun(
      { moduleName: c.string },
      { implementation: c.object() },
      { contracts: c.hash(c.contract) },
      { additionalExports: c.optional(c.object()) }
    )
    .doc(
      '`publish` does the same wrapping as `wrapAll`, then records any documentation',
  ...
  documentationTable: c
    .hash(
      c.object({
        doc: c.array(c.string),
        categories: c.array(
          c.object({ name: c.string, doc: c.array(c.string) })
        ),
    ...
    .doc(
      'A table of module names containing the documentation and contracts of',

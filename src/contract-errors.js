...
var stackContextItems = {
  argument: function(arg) {
    return {
      ...
      long:
        'for the ' +
        (_.isNumber(arg) ? u.ith(arg) : '`' + arg + '`') +
        ' argument of the call.',
    }
  },

  this: { short: '.this', long: 'for the `this` argument of the call.' },

  result: { short: '.result', long: 'for the return value of the call.' },

  extraArguments: {
    short: '.extraArguments',
...
ContractError.prototype = _.extend(Object.create(Error.prototype), {
  ...
  blame: function(context) {
    ...
    var thingNameWithParens =
      self.context.thingName +
      (self.context.contract.isFunctionContract ? '()' : '')
  ...
  expected: function(expected, data, /*opt*/ context) {
    ...
    self.data = data
    self.message +=
      'Expected ' + expected + ', but got ' + u.stringify(data) + '\n'
    return self
  },
  ...
  fullContract: function(/*opt*/ context) {
    ...
    if (!_.isEmpty(self.context.stack)) {
      ...
      if (!_.isEmpty(stack)) {
        var stackStrings = _.map(stack, function(i) {
